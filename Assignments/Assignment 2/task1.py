...
conf = SparkConf().setAppName("Assignment2_Task1")
...
def load_data(file_path, case):
    ...
    # Depending on the case provided, transpose the data
    if case == 1:
        return data.groupByKey().mapValues(set)
    else:
        return data.map(lambda x: (x[1], x[0])).groupByKey().mapValues(set)
...
if __name__ == "__main__":
    ...
    # Load data based on the case number provided
    baskets = load_data(input_file_path, case_number)
